...
write.table(OUT,file='./RESULT/Heatmap_zvalue_withZcut.txt',sep='\t',row.names=T,col.names=T,quote=F)



pdf('./RESULT/Heatmap_withZcut.pdf',width=5,height=4)
...
write.table(OUT,file='./RESULT/GENE_CLUSTER_withZcut.txt',sep='\t',row.names=T,col.names=F,quote=F)
...
used_gene_index=which(MAX_RPKM >1 & MAX_FD > 2)
...
OUT=cbind(o.split,o.mat)
colnames(OUT)[1]='GENE\tCLUSTER'
write.table(OUT,file='./RESULT/Heatmap_zvalue_withoutZcut.txt',sep='\t',row.names=T,col.names=T,quote=F)



pdf('./RESULT/Heatmap_withoutZcut.pdf',width=5,height=4)
...
OUT=NEW.C[order(NEW.C)]
write.table(OUT,file='./RESULT/GENE_CLUSTER_withoutZcut.txt',sep='\t',row.names=T,col.names=F,quote=F)
